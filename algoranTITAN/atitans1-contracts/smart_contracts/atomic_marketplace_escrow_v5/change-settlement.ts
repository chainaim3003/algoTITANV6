/**
 * Change Settlement Currency for AtomicMarketplaceEscrowV5
 * 
 * Allows creator to change the settlement currency after initialization
 */

import { AlgorandClient } from '@algorandfoundation/algokit-utils'
import { AtomicMarketplaceEscrowV5Client } from '../artifacts/atomic_marketplace_escrow_v5/AtomicMarketplaceEscrowV5Client'
...
const V5_ESCROW_APP_ID = 0  // Update with your deployed app ID
...
async function changeSettlementCurrency() {
  ...
  console.log(`   App ID: ${V5_ESCROW_APP_ID}`)
  
  if (V5_ESCROW_APP_ID === 0) {
    console.error('❌ Please update V5_ESCROW_APP_ID with your deployed app ID')
    process.exit(1)
  }
  ...
  // Get typed client
  const appClient = algorand.client.getTypedAppClientById(AtomicMarketplaceEscrowV5Client, {
    appId: BigInt(V5_ESCROW_APP_ID),
  ...
  const appInfo = await algodClient.getApplicationByID(V5_ESCROW_APP_ID).do()
  ...
  try {
    ...
    // Verify change
    console.log('\n📊 Verifying change...')
    const updatedAppInfo = await algodClient.getApplicationByID(V5_ESCROW_APP_ID).do()
